...
public class CoordinateEnrichmentStep implements PipelineStep {
    ...
    public void execute(PipelineContext context) throws Exception {
        ...
        List<Coordinate> validCoords = filterValidCoordinates(context.getInputCoordinates(), context.getPrintArea());
...
//                .filter(e -> e.getType() != ElementType.STATIC_TEXT || (e.getContent() != null && !e.getContent().isBlank()))
    ...
    private List<Coordinate> filterValidCoordinates(List<Coordinate> coordinates, PrintArea printArea) {
        if (coordinates == null) return List.of();
        return coordinates.stream()
                .filter(c -> c.getId() != null && !c.getId().equals("no-id"))
                .map(c -> toPrintAreaCoordinates(c, printArea))
                .filter(c -> c != null)
                .toList();
    }

    private Coordinate toPrintAreaCoordinates(Coordinate coordinate, PrintArea printArea) {
        if (printArea == null) return coordinate;
        if (printArea.getWidth() <= 0 || printArea.getHeight() <= 0) return coordinate;

        int left = Math.max(coordinate.getX(), printArea.getX());
        int top = Math.max(coordinate.getY(), printArea.getY());
        int right = Math.min(coordinate.getX() + coordinate.getWidth(), printArea.getX() + printArea.getWidth());
        int bottom = Math.min(coordinate.getY() + coordinate.getHeight(), printArea.getY() + printArea.getHeight());
    ...
    private boolean hasGeometry(JrxmlElement element) {
        boolean hasGeometry = element.getX() != null
                && element.getY() != null
                && element.getWidth() != null
                && element.getHeight() != null;

        if (!hasGeometry) {
            System.out.printf("[Enrich] SKIP WITHOUT GEOMETRY: elementId=%s content=%s%n",
                    element.getElementId(), element.getContent());
        }
