...
Папка дистрибутива - это папка, в которой лежат скрипты `scripts/pre-start.sh`, `scripts/start.sh` и `scripts/stop.sh`. Все служебные файлы запуска лежат в корне этой папки:
...
Для запуска нужен файл `env` в корне папки дистрибутива:
...
4. Выполните детерминированные проверки перед запуском:

```bash
./scripts/pre-start.sh
```

`pre-start.sh` проверяет наличие и размер `jasper_ai.jar`, env-файл, нужный API-ключ, доступность логов и Java 17+. Если проверка завершилась ошибкой, исправьте причину из `JASPERAI_NEXT_STEP` и не запускайте приложение до повторной успешной проверки.
...
`start.sh` запускает Java-приложение и пишет логи в `logs/jasperai.log` в той же папке дистрибутива. После запуска проверьте, что порт `8080` слушает процесс JasperAI:
