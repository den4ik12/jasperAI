...
public class FrameGroupingStep implements PipelineStep {
    ...
    private Comparator<JrxmlFrame> getFramePrioritySorter() {
        return Comparator.comparingInt(frame -> frame.getElements().stream()
                .anyMatch(e -> e.getType() == ElementType.TEXT_FIELD || e.getType() == ElementType.STATIC_TEXT) ? 1 : 0);
    }
}
