...
Если `JASPERAI_ERROR_RESOLVER` просит создать или обновить `./env`, обязательно получи оба значения: провайдер и API-ключ. Провайдер можно выбрать только из двух вариантов: DeepSeek или GigaChat. API-ключ обязателен для выбранного провайдера. Нельзя пропускать провайдер, ключ или запускать приложение до повторного успешного `./scripts/pre-start.sh`. Не выводи ключ в чат.
